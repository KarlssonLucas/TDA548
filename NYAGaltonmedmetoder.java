...
public class Galtonmedmetoder {
	...
	public static void raderbollar(int bollar) {
		int f = bollar;
		int[] bollnummer = new int[bollar];
		for (int i = 0; i < bollnummer.length; i++) {
			bollnummer[i] = f;
			f=f-1;
		}
    ...
    // Här kommer loopen där printningen av bollarna sker, den funkar så att du kör loopen
    // lika många gånger som arrayen ballsposition är lång, sedan testar den på alla if satser och 
    // kommer antingen printa 0 eller space
    int t = 0;
    for (int i = ballsPosition.length; i >= 0; i--) {
        if (pos0 > i) {
            System.out.print(bollnummer[t]);
            t++;
        } else {
            System.out.print(" ");
        }
